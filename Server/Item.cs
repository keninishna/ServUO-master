...
namespace Server
{
    ...
    public class BounceInfo
    {
        ...
        public static BounceInfo Deserialize(Database.Item a)
        {

            Map map = Map.Maps[(byte)a.BounceMap];
            Point3D loc = new Point3D((int)a.BounceLocX, (int)a.BounceLocY, (int)a.BounceLocZ);
                Point3D worldLoc = new Point3D((int)a.BounceWLocX,(int)a.BounceWLocY,(int)a.BounceWLocZ);

                object parent;

                Serial serial = (Serial)(int)a.BounceParentSerial;
        ...
        public static Database.Item Serialize(BounceInfo info, Database.Item a)
        {

            a.BounceMap = (byte)(info.m_Map.MapIndex);
            a.BounceLocX = (info.m_Location.X);
            a.BounceLocY = (info.m_Location.Y);
            a.BounceLocZ = (info.m_Location.Z);

            a.BounceWLocX = (info.m_WorldLoc.X);
            a.BounceWLocY = (info.m_WorldLoc.Y);
            a.BounceWLocZ = (info.m_WorldLoc.Z);

            if (info.m_Parent is Mobile)
            {
                a.BounceParentSerial = ((Mobile)info.m_Parent).Serial.Value;
                a.BounceParentType = true;
            }
            else if (info.m_Parent is Item)
            {
                a.BounceParentSerial = ((Item)info.m_Parent).Serial.Value;
                a.BounceParentType = false;
            }
            else
            {
                a.BounceParentSerial = 0;
            }
	...
	public class Item : IEntity, IHued, IComparable<Item>, ISerializable, ISpawnable
	{
        ...
        public virtual Database.Item Serialize(Database.Item v)
        {
            ...
            if (GetSaveFlag(flags, SaveFlag.Bounce))
            {
                v = BounceInfo.Serialize(info.m_Bounce, v);
            }


            if (GetSaveFlag(flags, SaveFlag.LootType))
            {
                v.m_LootType = (byte)m_LootType;
            }
            ...
            if (GetSaveFlag(flags, SaveFlag.Layer))
            {
                v.m_Layer = (byte)m_Layer;
            }

            if (GetSaveFlag(flags, SaveFlag.Name))
            {
                v.m_Name = info.m_Name;
            }
            ...
            if (GetSaveFlag(flags, SaveFlag.Parent))
            {
                if (m_Parent is Mobile && !((Mobile)m_Parent).Deleted)
                {
                    v.m_Parent = ((Mobile)m_Parent).Serial.Value;
                    v.ParentType = true;
                }
                else if (m_Parent is Item && !((Item)m_Parent).Deleted)
                {
                    v.m_Parent = ((Item)m_Parent).Serial.Value;
                    v.ParentType = false;
                }
                else
                {
            ...
            if (GetSaveFlag(flags, SaveFlag.Items))
            {
                ...
                {
                    if (item == null || item.Deleted) v.m_Items += "-1;";
                    v.m_Items += item.Serial.Value + ";";
                }
            }
            ...
            if (GetSaveFlag(flags, SaveFlag.ImplFlags))
            {
                v.m_ImplFlags = (int)implFlags;
            }

            if (GetSaveFlag(flags, SaveFlag.BlessedFor))
            {
                v.m_BlessedFor = (info.m_BlessedFor.Serial.Value);
            }

            if (GetSaveFlag(flags, SaveFlag.HeldBy))
            {
                v.m_HeldBy = (info.m_HeldBy.Serial.Value);
            }

            if (GetSaveFlag(flags, SaveFlag.SavedFlags))
            {
                v.m_SavedFlags = info.m_SavedFlags;
            }

            v.m_Description = (this).GetType().FullName;
            v.m_Direction = (byte)m_Direction;
            v.m_Visible = Visible;
            v.m_Stackable = Stackable;
            v.m_Movable = Movable;
		...
		public virtual void Deserialize(GenericReader reader)
		{
            if (Core.UseSQLLoad)
            {
                SaveFlag flags = (SaveFlag)this.TempFlags;
                this.TempFlags = 0;
                return;
            }
        ...
        public virtual void Deserialize(Database.Item c)
        {


            SetLastMoved();

                SaveFlag flags = (SaveFlag)(int)c.flags;
            this.TempFlags = (int)flags;
                LastMoved = DateTime.UtcNow; //should fix this i guess
                ...
                if (GetSaveFlag(flags, SaveFlag.Bounce))
                {
                    AcquireCompactInfo().m_Bounce = BounceInfo.Deserialize(c);
                }

                if (GetSaveFlag(flags, SaveFlag.LootType))
                {
                    m_LootType = (LootType)c.m_LootType;
                }
            ...
            if (GetSaveFlag(flags, SaveFlag.Parent))
            {
                    Serial parent = (Serial)(int)c.m_Parent;
            ...
            if (GetSaveFlag(flags, SaveFlag.Items))
            {
                string[] itemlist = ((string)c.m_Items).Split(new string[1] { ";" }, StringSplitOptions.RemoveEmptyEntries);
                var items = new List<Item>();
                foreach (string i in itemlist)
                {
                    int p = 0;
                    Int32.TryParse(i, out p);
                    items.Add(World.FindItem((Serial)p));
                }
                ...
                double weight = (double)c.m_Weight;
                ...
                if (GetSaveFlag(flags, SaveFlag.Visible))
                {
                    SetFlag(ImplFlag.Visible, (bool)c.m_Visible);
                }
                else
                {
                    SetFlag(ImplFlag.Visible, true);
                ...
                if (GetSaveFlag(flags, SaveFlag.Movable))
                {
                    SetFlag(ImplFlag.Movable, (bool)c.m_Movable);
                }
                else
                {
                    SetFlag(ImplFlag.Movable, true);
                ...
                if (GetSaveFlag(flags, SaveFlag.Stackable))
                {
                    SetFlag(ImplFlag.Stackable, (bool)c.m_Stackable);
                }

                if (GetSaveFlag(flags, SaveFlag.ImplFlags))
                {
                    m_Flags = (ImplFlag)(int)c.m_ImplFlags;
                }
                ...
                if (GetSaveFlag(flags, SaveFlag.BlessedFor))
                {
                    AcquireCompactInfo().m_BlessedFor = World.FindMobile((int)c.m_BlessedFor);
                }

                if (GetSaveFlag(flags, SaveFlag.HeldBy))
                {
                    AcquireCompactInfo().m_HeldBy = World.FindMobile((int)c.m_HeldBy);
                }

                if (GetSaveFlag(flags, SaveFlag.SavedFlags))
                {
                    AcquireCompactInfo().m_SavedFlags = (int)c.m_SavedFlags;
                }
